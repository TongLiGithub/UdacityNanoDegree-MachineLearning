...
The dataset used in this project contains financial and email information of 144 Enron employees, with 18 POI among them. Missing values exist in most of the features. More information will be given for the features I selected to use in the POI identifier later. 
...
### Answer:
I used seven features in my POI identifier, i.e., 'bonus_total_ratio', 'total_stock_value', 'exercised_stock_options', 'to_messages', 'from_messages', 'from_email_poi_proportion', and 'to_email_poi_proportion'. Among them, 'total_stock_value', 'exercised_stock_options', 'to_messages', and 'from_messages' are originally in the dataset. They were selected in terms of the scatter plot between each of these variables and the poi variable: if the variable does not have a lot of overlap for POI (poi=True) and non-POI (poi=False). The other three are new variables. The variable 'bonus_total_ratio' was created using 'bonus' divided by 'total_payments'. I expected that POI tend to have higher bonus_total_ratio compared to non-POI. The variable 'from_email_poi_proportion' was created with 'from_this_person_to_poi' divided by 'from_messages', and similarly, 'to_email_poi_proportion' was created with 'from_poi_to_this_person' divided by 'to_messages'. I expected that POI probably have more frequent contact with other POI, and their emails with other POI should take a larger proportion among all their emails.

Among the seven features, 'exercised_stock_options' is the only one with no missing values. To be specific, 'bonus_total_ratio' has 63 missing values and 'total_stock_value' has 19 missing values, 'to_messages', 'from_messages', 'from_email_poi_proportion' and 'to_email_poi_proportion' have 58 missing values each. 
...
I used SelectKBest to select from the seven features. The feature scores are as follows: 'bonus_total_ratio': 4.36261288, 'total_stock_value': 16.3056319, 'exercised_stock_options': 15.3297295, 'to_messages': 0.382453942, 'from_messages': 0.0943681508, 'from_email_poi_proportion': 5.74486543, and 'to_email_poi_proportion': 0.00138845322. Among them, 'bonus_total_ratio', 'total_stock_value', 'exercised_stock_options' and 'from_email_poi_proportion' have p values smaller than 0.05, so they were used in the Decision Tree for the machine learning algorithm.
...
### Answer:
I ended up using Decision Tree classifier. I also tried SVM, GaussianNB, RandomForest, and Adaboost. The F1 scores of these five algorithms are from about 0.25 to 0.45, with GaussianNB < Adaboost < Random Forest < SVM < Decision Tree. SVM has very high precision but low recall, which is in contrast to GaussianNB which has high recall but low precision. Adaboost has relatively low values on both of the two evaluation metrics. Random Forest has a medium precision, but low recall. Decision Tree is the only one that has balanced precision and recall, both of which are medium (approximately 0.3 to 0.4).
...
### Answer:
The Deicision Tree classifier gives a precision about 0.30, and a recall score about 0.40. It means that for all the individuals identified as POI by the algorithm, 30% are real POI (precision), and for all the POI in the real world, 40% of them are correctly identified by the algorithm.  